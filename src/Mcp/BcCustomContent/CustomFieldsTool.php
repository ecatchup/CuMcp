            ...
            ->withTool(
                ...
                inputSchema: [
                    ...
                    'properties' => [
                        ...
                        'type' => ['type' => 'string', 'enum' => self::TYPES, 'description' => 'フィールドタイプ（必須）'],
                        'status' => ['type' => 'number', 'description' => 'ステータス（0: 無効, 1: 有効）（初期値は1）'],
                        'defaultValue' => ['type' => 'string', 'description' => 'カスタムエントリーの入力欄の初期値'],
                        'validate' => ['type' => 'string', 'enum' => self::VALIDATION_RULES, 'description' => 'バリデーションルール（配列で複数選択可）'],
            ...
            ->withTool(
                ...
                inputSchema: [
                    ...
                    'properties' => [
                        ...
                        'type' => ['type' => 'string', 'enum' => self::TYPES, 'description' => 'フィールドタイプ'],
                        'status' => ['type' => 'number', 'description' => 'ステータス（0: 無効, 1: 有効）（初期値は1）'],
                        'defaultValue' => ['type' => 'string', 'description' => 'カスタムエントリーの入力欄の初期値'],
                        'validate' => ['type' => 'string', 'enum' => self::VALIDATION_RULES, 'description' => 'バリデーションルール（配列で複数選択可）'],
                        'regex' => ['type' => 'string', 'description' => '正規表現バリデーション（正規表現でバリデーションを実行したい場合に指定する）'],
