...
class McpServerToolCallTest extends BcTestCase
{
    ...
    public function testCallToolAddBlogPost()
    {
        ...
        [$result, $isError] = $this->callTool('addBlogPost', [
            ...
            'name' => 'about-cumcp',
            'status' => 0,
            'content' => '<p>CuMcp（Catchup MCP）は、baserCMSを外部のAIエージェントから直接操作できるようにするMCP（Model Context Protocol）サーバーです。ブログ記事やカテゴリ、タグの管理はもちろん、カスタムテーブル・カスタムコンテンツ・カスタムエントリー・カスタムリンクといったbaserCMSの柔軟な拡張機能まで、AIアシスタント経由で読み書きできます。</p>',
        ...
        $this->assertEquals('CuMcpについて', $result['title']);
        $this->assertEquals('about-cumcp', $result['name']);
    ...
    private function getDetail(): string
    {
        return <<<EOF
<h2>CuMcpとは</h2>
<p>CuMcp（Catchup MCP）は、baserCMSをAIエージェントから直接操作するためのMCP（Model Context Protocol）サーバーです。MCPは、AIアシスタントと外部システムを標準化された方法でつなぐプロトコルであり、CuMcpはこの仕組みを使ってbaserCMSのAPIをAIエージェント向けに公開しています。</p>
<p>これにより、ChatやAIエージェントとの対話の中で「ブログ記事を書いて」「このカテゴリを追加して」といった指示を出すだけで、baserCMSサイトの更新が完結するようになります。</p>

<h2>CuMcpでできること</h2>
<p>CuMcpは、baserCMSが持つ主要な機能をひととおりカバーしています。</p>
<ul>
...
<h2>なぜCuMcpを作ったのか</h2>
<p>baserCMSは2010年の誕生以来、オープンソースのCMSとして進化を続けてきました。近年のAIエージェントの普及を受けて、baserCMSを「人が管理画面を操作するCMS」から一歩進めて、「AIエージェントが自律的に運用できるCMS（Agentic CMS）」として位置づけ直す取り組みの一環がCuMcpです。</p>
<p>管理画面にログインして手作業で更新する代わりに、AIエージェントに指示を出すだけでサイト更新が完結する。CuMcpはそのための土台となるインターフェースです。</p>

<h2>活用イメージ</h2>
<p>ClaudeのようなAIアシスタントにCuMcpを接続すると、たとえば次のようなことが会話ベースで行えるようになります。</p>
<ul>
...
<h2>まとめ</h2>
<p>CuMcpは、baserCMSをAIエージェントから直接操作できるようにするMCPサーバーです。ブログ管理からカスタムテーブルを使った独自コンテンツ管理まで幅広くカバーしており、baserCMSをAIネイティブに運用していくための重要なピースとなっています。</p>
EOF;
    }
